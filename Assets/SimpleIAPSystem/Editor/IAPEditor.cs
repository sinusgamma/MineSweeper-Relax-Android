        ...
        void DrawIAP(List<IAPGroup> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                    ...
                    switch (productSelection)
                    {
                        case 1:
                            ...
                            for (int j = 0; j < platforms; j++)
                                newObj.localIDs.Add(new IAPIdentifier());
                            break;
                        case 2:
                ...
                for (int j = 0; j < group.items.Count; j++)
                {
                    ...
                    obj.type = (UnityEngine.Purchasing.ProductType)EditorGUILayout.EnumPopup(obj.type, GUILayout.MaxWidth(110));
					...
					if (obj.isVirtual) 
					{
						if(obj.type == UnityEngine.Purchasing.ProductType.Subscription)
						{
							Debug.LogWarning("Subscriptions are not available for virtual products. Resetting to Consumable.");
							obj.type = UnityEngine.Purchasing.ProductType.Consumable;
						}
