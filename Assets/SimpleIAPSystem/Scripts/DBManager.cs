    ...
    public class DBManager : MonoBehaviour
    {
        ...
        void InitDB()
        {
            ...
            for (int i = 0; i < IAPs.Length; i++)
            {
                ...
                {
                    //initialize new non consumable product or subscription as not purchased
                    if (obj.type == ProductType.NonConsumable || obj.type == ProductType.Subscription)
                    {
                        //start bool as purchased, then check requirements
                        bool startsPurchased = true;

                        switch (obj.isVirtual)
                        {
                            case false:
                ...
                if (!string.IsNullOrEmpty(node["type"]))
                {
                    switch (node["type"])
                    {
                        case "consumable":
                            ...
                            break;
                        default:
                            obj.type = ProductType.NonConsumable;
                            break;
                    }
                }
